  ...
  const onSubmit = async (formData: TForm) => {
    ...
    const createPromise = quickAddCallback(projectId.toString(), payload).then(async (created) => {
      ...
      try {
        ...
        // Create already set parent_id; this only refreshes the L3 child list
      }
      return created;
    });
  ...
  if (!isOpen || !selectedType) {
    return (
      <CustomMenu
        ...
        customButton={
          <div className="flex w-full cursor-pointer items-center gap-2 rounded-lg bg-layer-transparent px-2 py-1.5 hover:bg-layer-transparent-hover">
