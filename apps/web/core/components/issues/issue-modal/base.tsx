...
export const CreateUpdateIssueModalBase = observer(function CreateUpdateIssueModalBase(props: IssuesModalProps) {
  ...
  const addIssueToCycle = async (issue: TIssue, selectedCycleId: string) => {
    if (!workspaceSlug || !issue.project_id) return;

    await issues.addIssueToCycle(workspaceSlug.toString(), issue.project_id, selectedCycleId, [issue.id]);
    fetchCycleDetails(workspaceSlug.toString(), issue.project_id, selectedCycleId);
  };
  ...
  const addIssueToModule = async (issue: TIssue, moduleIds: string[]) => {
    ...
    await Promise.all([
      ...
      ...moduleIds.map(
        (selectedModuleId) =>
          issue.project_id && fetchModuleDetails(workspaceSlug.toString(), issue.project_id, selectedModuleId)
      ),
    ]);
  };
  ...
  const handleModuleChange = async (issueData: Partial<TIssue>, payload: Partial<TIssue>) => {
    ...
    for (const selectedModuleId of updatedModuleIds) {
      if (issueData.module_ids?.includes(selectedModuleId)) {
        modulesToRemove.push(selectedModuleId);
      } else {
        modulesToAdd.push(selectedModuleId);
      }
    }
